...
export const ExperienceTab: React.FC<ExperienceTabProps> = ({ onOpenExperience }) => {
  return (
    <div className="space-y-10 animate-in fade-in duration-300">
      ...
      {/* Title Header */}
      <div className="inline-block bg-white border-3 border-black px-6 py-2 rounded-2xl shadow-[4px_4px_0px_0px_#60A5FA] -rotate-1">
      ...
      <div className="space-y-8">
        {EXPERIENCES_DATA.map((exp) => (
          <div
            key={exp.id}
            className="bg-white rounded-3xl border-3 sm:border-4 border-black p-6 sm:p-8 shadow-[6px_6px_0px_0px_#000] hover:shadow-[8px_8px_0px_0px_#000] transition-all space-y-6"
          >
            {/* Top Info Bar */}
            ...
            <div className="space-y-2">
              ...
              <ul className="space-y-2">
                {exp.responsibilities.map((item, idx) => (
                  <li key={idx} className="text-xs sm:text-sm text-gray-800 font-medium flex items-start gap-2">
            ...
            {/* Highlight Metric */}
            {exp.metrics && (
              <div className="bg-[#D1FAE5] border-2 border-black p-3.5 rounded-xl font-bold text-black text-xs sm:text-sm flex items-center justify-between">
            ...
            <div className="flex flex-wrap items-center justify-between gap-4 pt-2 border-t-2 border-dashed border-gray-200">
              <div className="flex flex-wrap gap-1.5">
                {exp.techStack.map((tech, i) => (
                  <span key={i} className="px-2.5 py-1 bg-white border border-black rounded-lg text-xs font-bold shadow-[1px_1px_0px_0px_#000]">
                    {tech}
                  </span>
                ))}
              </div>
              ...
              <button
                ...
                className="neo-btn bg-black text-white px-4 py-2 rounded-xl text-xs font-bold flex items-center gap-1.5 hover:bg-gray-800"
              >
                <span>展开详细经历</span>
